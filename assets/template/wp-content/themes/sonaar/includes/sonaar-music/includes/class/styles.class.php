		...
		public function setFont($element, $option_key, $important = false) {
			...
			if($this->use_options) {
				...
				if(!empty($font) && is_string($font_family) && strpos($font, "_safe_") === false) {
					...
					if( isset($this->google_fonts[md5($font_family)]) && isset($this->google_fonts[md5($font_family)]['font-weight']) && is_string( $this->google_fonts[md5($font_family)]['font-weight'] ) ) {

						if ( strpos( $this->google_fonts[md5($font_family)]['font-weight'] , $this->get_option($option_key, 'weight')  ) === false) {
